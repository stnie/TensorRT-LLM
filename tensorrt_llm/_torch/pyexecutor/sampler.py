...
from typing import Any, Callable, Generic, List, NamedTuple, Optional, TypeVar, cast
...
class TorchSampler(Sampler):
    ...
    def _handle_stop_criteria(self, request: LlmRequest, new_token: int, beam_idx: int) -> bool:
        ...
        if new_token == request.py_end_id:
            ...
            return True
        if self._meet_max_token_stop_criteria(request, beam_idx):
            request.finish_by(FinishReason.LENGTH, beam_idx)
            return True
        if self._meet_stop_token_criteria(request, beam_idx):
    ...
    def _process_draft_tokens_greedy(
        ...
        new_token = add_token(request, new_tokens, beam_idx=self.DEFAULT_BEAM_IDX)
        stop = self._handle_stop_criteria(request, new_token, beam_idx=self.DEFAULT_BEAM_IDX)
        ...
        for draft_token in request.py_draft_tokens:
            ...
            num_accepted += 1
            new_token = add_token(
                request, new_tokens, beam_idx=self.DEFAULT_BEAM_IDX, step=num_accepted
            )
            if self._handle_stop_criteria(request, new_token, beam_idx=self.DEFAULT_BEAM_IDX):
                break
        return num_accepted
            ...
            if longest_accepted_len == 0:
                ...
                for idx in eagle_paths[longest_match_path_idx][:longest_accepted_len]:
                    ...
                    if self._handle_stop_criteria(
                        request, new_token, beam_idx=self.DEFAULT_BEAM_IDX
                    ):
                        break
    ...
    def _process_draft_tokens_rejection_sampling(
        ...
        for i in range(num_accepted):
            ...
            new_tokens_tensor[i, request.seq_slot, self.DEFAULT_BEAM_IDX] = new_token
            request.add_new_token(new_token, self.DEFAULT_BEAM_IDX)
            stop = self._handle_stop_criteria(request, new_token, beam_idx=self.DEFAULT_BEAM_IDX)
            if stop:
                num_accepted = i + 1
                return num_accepted
        if sample_last:
            ...
            new_tokens_tensor[num_accepted, request.seq_slot, self.DEFAULT_BEAM_IDX] = new_token
            request.add_new_token(new_token, self.DEFAULT_BEAM_IDX)
        else:
            new_token = add_token(
                request, new_tokens_list, beam_idx=self.DEFAULT_BEAM_IDX, step=num_accepted
            )
        stop = self._handle_stop_criteria(request, new_token, beam_idx=self.DEFAULT_BEAM_IDX)
